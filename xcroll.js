...
//ck
function xcrollck() {
   if ($request.url.indexOf("roll") > -1) {
    $.setdata($request.url,'xcrollurl')
    $.log(xcrollurl)
$.setdata(JSON.stringify($request.headers),'xcrollhd')
$.log(xcrollhd)
    $.setdata($request.body,'xcrollbody')
$.log(xcrollbody)
...
JSON.parse(xcrollhd),
    ...
    const result = JSON.parse(data)
        if(result.errcode == 0){
        console.log('\n丢骰子成功：'+result.errmsg+'丢出：'+result.random+'点')
}else{
        console.log('\n丢骰子失败：'+result.errmsg)
}
        } catch (e) {
