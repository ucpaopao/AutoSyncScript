...
const $ = new Env('携程10S');
let xcsmurl = $.getdata('xcsmurl')
let xcsmhd = $.getdata('xcsmhd')
let xcsmbody = $.getdata('xcsmbody')
let user_id = ''
!(async () => {
...
function xcsmck() {
   if ($request.url.indexOf("userAcceptPrize") > -1) {
    $.setdata($request.url,'xcsmurl')
    $.log(xcsmurl)
$.setdata(JSON.stringify($request.headers),'xcsmhd')
$.log(xcsmhd)
    $.setdata($request.body,'xcsmbody')
$.log(xcsmbody)
   $.msg($.name,"","携程10Sbody成功！")
    } 
  }

//签到  
function xcsmqd(timeout = 0) {
  return new Promise((resolve) => {
//user_id=xcsmurl.match(/user_id=(\d+)/)[1]
//let url = {url : `https://m.ctrip.com/restapi/soa2/20794/gatherCoin?`,
  let url = {url : xcsmurl,
        headers : 
JSON.parse($.getdata('xcsmhd')),
