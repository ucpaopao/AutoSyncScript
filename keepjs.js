...
let keepjsurl = $.getdata('keepjsurl')
let keepjshd = $.getdata('keepjshd')
let keepjsbody = $.getdata('keepjsbody')
//let user_id = ''
!(async () => {
...
function keepjsck() {
   if ($request.url.indexOf("gainEnergy") > -1) {
    $.setdata($request.url,'keepjsurl')
    $.log(keepjsurl)
$.setdata(JSON.stringify($request.headers),'keepjshd')
$.log(keepjshd)
    $.setdata($request.body,'keepjsbody')
$.log(keepjsbody)
